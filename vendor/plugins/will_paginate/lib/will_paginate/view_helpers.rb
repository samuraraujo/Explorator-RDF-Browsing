...
module WillPaginate
    ...
    def page_entries_info(collection, options = {})
      ...
      if collection.total_pages < 2
        case collection.size
          ...
          else;   "<b>Todos #{collection.size}</b> #{entry_name.pluralize}"
        end
      else
        %{<b>%d&nbsp;à&nbsp;%d</b> de <b>%d</b> #{entry_name.pluralize} } % [
        collection.offset + 1,
  ...
  class LinkRenderer
    ...
    #    end
    #   
    #This code was adapted to explorator.
    def page_link_or_span(page, span_class = 'current', text = nil)
      text ||= page.to_s
      if page and page != current_page
        if @remote
          # @template.link_to_remote text, {:url => params.rec_merge!(:page => page)}
          @remote[:url] = @remote[:url].merge(:page => page)
          @template.link_to_remote (text, @remote).gsub('new Ajax.Updater','javascript:ajax_update')
        else
